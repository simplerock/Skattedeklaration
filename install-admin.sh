...
PROJECT="$HOME/Desktop/skattedeklaration"

if [ ! -d "$PROJECT" ]; then
  echo "❌ Hittade inte projektet på $PROJECT"
  echo "   Kontrollera att Next.js-projektet finns på ~/Desktop/skattedeklaration"
  exit 1
fi

echo "📁 Skriver filer till $PROJECT..."
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', alignItems: 'center', height: 56, gap: 8 }}>
          ...
          <div style={{ marginLeft: 'auto' }}>
            <input
              ...
              placeholder="Sök..."
              style={{ padding: '6px 12px', border: '1px solid #e5e7eb', borderRadius: 6, fontSize: 13, outline: 'none', width: 200, background: '#f9fafb', color: '#111827' }}
            />
          </div>
        </div>
      </div>
                  ...
                  return (
                    <>
                      <tr
                        ...
                        style={{ borderBottom: isExpanded ? 'none' : '1px solid #f3f4f6', background: isExpanded ? '#fafafa' : '#fff', cursor: 'pointer' }}
                      ...
                      >
                        ...
                        <td style={{ padding: '12px 14px', fontFamily: 'monospace', fontSize: 11, color: '#2563eb', fontWeight: 600 }}>{r.reference_number}</td>
                        <td style={{ padding: '12px 14px', fontWeight: 500, color: isDone ? '#9ca3af' : '#111827', textDecoration: isDone ? 'line-through' : 'none', whiteSpace: 'nowrap' }}>{r.first_name} {r.last_name}</td>
                        <td style={{ padding: '12px 14px', fontFamily: 'monospace', fontSize: 12, color: '#9ca3af' }}>{r.personal_identity_number}</td>
                        <td style={{ padding: '12px 14px', color: '#6b7280' }}>{r.income_year}</td>
                        <td style={{ padding: '12px 14px', textAlign: 'right', fontWeight: 500, color: totalAvdrag > 0 ? '#15803d' : '#9ca3af' }}>{krFmt(totalAvdrag)}</td>
                        <td style={{ padding: '12px 14px', textAlign: 'right', color: '#9ca3af', whiteSpace: 'nowrap' }}>{timeAgo(r.created_at)}</td>
                        <td style={{ padding: '12px 16px 12px 14px', textAlign: 'right' }}>
                          <button
                            ...
                            style={{
                              ...
                              cursor: downloading === r.id ? 'default' : 'pointer', whiteSpace: 'nowrap',
                            }}
                          >
                      ...
                      {isExpanded && (
                        <tr key={`${r.id}-detail`} style={{ borderBottom: '1px solid #f3f4f6', background: '#fafafa' }}>
                          <td colSpan={8} style={{ padding: '0 16px 16px 16px' }}>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12, padding: '14px 0 2px', borderTop: '1px solid #f3f4f6' }}>
                              {[
                                ...
                                { label: 'Uthyrning',       value: krFmt(r.rental_net) },
...
# ── 2. Status API route ────────────────────────────────────────────────────────
mkdir -p "$PROJECT/app/api/admin/declarations/[id]/status"
cat > "$PROJECT/app/api/admin/declarations/[id]/status/route.ts" << 'ENDOFFILE'
...
echo ""
echo "✅ Klart! Filerna är på plats:"
echo "   → app/admin/page.tsx"
echo "   → app/api/admin/declarations/[id]/status/route.ts"
echo ""
echo "   Gör CMD+SHIFT+R på http://localhost:3000/admin"
